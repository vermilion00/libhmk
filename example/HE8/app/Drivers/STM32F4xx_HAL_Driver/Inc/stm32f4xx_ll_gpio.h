...
typedef struct {
    ...
    uint32_t
        Pull; /*!< Specifies the operating Pull-up/Pull down for the selected
...
__STATIC_INLINE uint32_t LL_GPIO_GetPinMode(GPIO_TypeDef *GPIOx, uint32_t Pin) {
    return (uint32_t)(READ_BIT(GPIOx->MODER, (GPIO_MODER_MODER0
                                              << (POSITION_VAL(Pin) * 2U))) >>
                      (POSITION_VAL(Pin) * 2U));
}
...
__STATIC_INLINE uint32_t LL_GPIO_GetPinPull(GPIO_TypeDef *GPIOx, uint32_t Pin) {
    return (uint32_t)(READ_BIT(GPIOx->PUPDR, (GPIO_PUPDR_PUPDR0
                                              << (POSITION_VAL(Pin) * 2U))) >>
                      (POSITION_VAL(Pin) * 2U));
}
